...
// "Show more" button sits BELOW the first 5 rows.
// Clicking it reveals remaining rows BELOW the button, then button becomes "Show less".
// The button never floats up — it always stays at the bottom of the list.
function StockDropdown({ stocks, onStockClick }) {
  const [open, setOpen] = useState(false);
  return (
    <div style={{marginBottom:6}}>
      {/* Extra rows — rendered BEFORE the button so button stays at bottom */}
      {open && stocks.map((s,i) => (
        <StockRow key={s.symbol+i} s={s} i={i} onStockClick={onStockClick}/>
      ))}
      <button
        onClick={() => setOpen(o => !o)}
        style={{
          ...
          >
            <div style={{flex:1, minWidth:0}}>
              <div style={{display:"flex", alignItems:"center", gap:5, marginBottom:5, flexWrap:"wrap"}}>
                {companyName && <span style={{fontWeight:800, fontSize:11, color:"var(--text)"}}>{companyName}</span>}
                ...
                {a.sentiment && a.sentiment!=="neutral" && (
                  <span style={{fontSize:9, fontWeight:700, color:a.sentiment==="bullish"?"var(--bull)":"var(--bear)"}}>
                    {a.sentiment==="bullish"?"▲ BULLISH":"▼ BEARISH"}
                  </span>
                )}
              </div>
              <p style={{fontWeight:600, fontSize:13.5, lineHeight:1.4, color:"var(--text)", margin:0, overflow:"hidden", display:"-webkit-box", WebkitLineClamp:2, WebkitBoxOrient:"vertical"}}>{a.headline}</p>
            </div>
...
export default function CategoryFeedPage({ categoryId, onBack }) {
  ...
  function handleStockClick(s) {
    window.__dashboardBackOverride = () => {
      if (window.__setActiveFilter) window.__setActiveFilter("cat:" + categoryId);
      delete window.__dashboardBackOverride;
    };
    if (window.__openDashboard) window.__openDashboard(s.symbol, getDisplayName(s));
  }
  ...
  async function loadCategory() {
    try {
      ...
      const d   = await res.json();
      if (d.success && d.data?.length) {
        setStocks(d.data); setLoadingS(false);
        fetchNews(d.data);
        return;
      }
